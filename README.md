...
打开编辑器 → 拖入 GLB → 自动体检 → 层级树 → 改变换/材质 → 存视点 → 做补间动画 →
加热点 → 建变量和规则 → 预览里点一下看规则跑 → 发布成 `.w3p` → 用播放器打开，行为一致。

这十二步是 v0 的定义，也是 `pnpm test:e2e` 逐步覆盖的内容。
