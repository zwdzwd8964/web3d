...
| **v0.5 · 断网可用（C6）** | ✅ **已真实执行（比原计划更强）** | 原计划是「断网构建 + 断网 `pnpm dev` 冒烟」。实际做的是黄金路径 II 第 ⑫ 步：发布出真 `.w3p` → Player 打开 → **拦截并断掉所有非本机源** → 重载 → 断言画面仍然起来且 `被拦截的请求数 == 0`。这比"我的机器上拔网线还能跑"强：它证明的是**发布出去的包一个外部地址都不请求**，而不是"这台机器上恰好都有缓存" | — |
| **v0.5 · 内置库零外链与许可齐全** | ✅ **已真实执行** | `check-library-manifest.mjs` 已挂进 `pnpm check:constitution`（T-173）：manifest 里任何 `http(s)://` 直接 fail，`license` 缺失直接 fail，总量超 40MB 直接 fail | — |
| **v0.5 · Player 体积** | ✅ **已测量** | gzip **243.0 KB / 400 KB**，余量 157 KB。HDRI、媒体、库内容全部走资产管线，不进 bundle——这正是本项要验证的，实测数字支持它 | — |
| v0.5 · 真实音频输出 | **未验证，且不打算验证** | CI 没有声卡。E2E 断言的是运行时播放状态（`isMediaPlaying`），不是声音。「真的响了」在无声卡环境里观测不到，硬测只会做出一个自信满满的 flaky 测试（进化规划 §2 原话） | 人工戴耳机跑一次 |
| v0.5 · 真机 GPU 上的灯光/阴影帧率 | **未测量** | SwiftShader 的数字与真实 GPU 无可比性 | T-174 的 bench 页 + H1 人工项（G0.5-8） |
