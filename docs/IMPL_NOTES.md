...
### D-02 · ⑥ 是另一个根因，而且它是**铁律 11 的真实违反** — ✅ **已修（2026-08-11）**
...
**照上面那句做的，那条测试第一次跑确实是红的**：「applyPatch 回落到全量重建（第 1 次）：
2 条 patch 未被识别」。补丁形状也与预测逐字一致（实测抓的，不是推的）：3 条位移 `replace`
（index 9/10/11）+ 尾部 5 条 `remove`（16/15/14/13/12），而 12 / 13 正是盖螺栓 3 / 4。

**修法**：`removeNode` 返回 false 有两个完全不同的意思，以前被当成同一件事——

| | 意思 | 该怎么办 |
|---|---|---|
| (a) | 它刚刚**随祖先一起**被删掉了 | 不是失败。同一件事在一批补丁里被算了两次 |
| (b) | 图与文档**本来就不同步** | 必须回落，那是报警器存在的全部理由 |

判据走 `prev` 的父链：只要有一个祖先也从 `next` 里消失了，就是 (a)。

**四条变异全红**，两条测试各守一个方向（修太松 → 报警器那条红；修太紧 → 主断言红）。
⚠ 其中一条第一次是绿的：成环那条的第一版夹具把环上两个节点都从 `next` 里拿掉，
于是父链第一步就短路，**根本没走进环**。换夹具之后才红。

**`golden-path.spec.ts` ⑥ 已转绿**（1.9s）。六条红变五条，剩下的五条是 D-01 那一组。
