...
| U-02 | IndexedDB 的配额、跨标签页锁 | ✅ **已真实执行**（T-202 · T-287 · T-289） | 配额路径由契约套件两侧同跑；跨标签页占用由 `drafts` facet 的会话租约实现，契约套件两侧各四条判定，**E2E 第 3 条开两个真标签页**断言 B 拿不到、A 的文档没被覆盖 | — |
...
| U-23 | **v1.0 · 剖切 × 描边的两处耦合（T-252 实测，已有结论）** | ✅ **已实测**（2026-08-05） | 在真浏览器里量了三个数（`e2e/tests/section-outline.spec.ts`）：**① 剖切在两条渲染路径上都生效**——加一把刀，直连改变 183212 像素、composer 改变 39012 像素，两者都远大于 0；**② 但 composer 那个数只有直连的 21%**，与「被剖掉的那一侧仍然画着轮廓」一致——成因在 three 里读得出来：`OutlinePass` 的 mask / depth 材质是 `ShaderMaterial` 且没设 `clipping: true`，而 `WebGLRenderer` 只对非 ShaderMaterial 或 `clipping === true` 绑裁剪 uniform；**③ 每加一条裁剪平面触发一次全量着色器重编译**（programs 2 → 9 → 16），因为 three 把平面**数量**放进 program 的 cache key | — |
| U-24 | 三档阴影在**目标机器**上的代价 | ❌ **未实测** | 本仓库开发与 CI 环境只有 SwiftShader 软件渲染，阴影贴图从 512 到 2048 的代价在软渲下与真实 GPU 无可比性。出厂默认 `medium` 今天是**拍的，不是测的** | M2 到手时，照 §S3 的实验设计跑一遍（与 T-291 同属出口人工验收项 H1） |
...
### T-252 · 描边不吃裁剪，是 v1.0 的已知限制

**结论**：同时开启剖切与描边时，**被剖掉的那一侧仍然会画出轮廓**。剖切本身在两条渲染
路径上都生效（composer 路径上加一把刀确实改变了 39012 像素），但描边通道不参与裁剪。

**为什么不修**：修它要改 three 的 `OutlinePass`——给它的 mask / depth 材质设 `clipping: true`
并自己传 uniform，等于在 vendor 里分叉一份 pass。v1.0 不做，登记为已知限制。

**用户看得见的地方有两处**：效果面板的描边段在场景里有剖切面时多一句中文提示；
完整性检查 I70（warn 级，规划 §4.2 编号的一次显式破例，产品负责人 2026-08-05 批准）。
...
`ShadowSchema.quality` 的 `.default('medium')` 从 v0.5 起就是这个值，而选它的依据是
「三档里居中的那一档」——不是任何一次测量。S3 要的就是把这个默认值从「看起来合理」
变成「在最低配机器上量过」。**今天量不了**（见 U-24），所以这一节写的是**实验设计**，
好让拿到机器的那半小时不用重新想。

### 实验设计（在 M2 上跑，约 15 分钟）

| 步 | 做什么 | 记什么 |
|---|---|---|
| 1 | 确认硬件加速开着（`chrome://gpu`） | 截图。**关着就整个作废** |
| 2 | 打开 bench 页，载入 `泵组样板.w3p` | — |
| 3 | 跑一轮，读「动态灯上限（开阴影）」三档 | 三个数 |
| 4 | 编辑器里把一盏聚光灯的阴影依次设 low / medium / high，各截一张**阴影边缘**的放大图 | 三张图 |
| 5 | 同一视角下读三档的平均帧率与 P95 | 三对数 |

### 判据（先写死，免得看到数字之后再挑标准）

**推荐档 = 满足下面两条的最高一档**：

1. 平均帧率 ≥ 45 fps（商务建议线的绿灯）；
2. 阴影边缘在 1080p 下**看不出明显锯齿**——这一条是人判的，所以第 4 步的三张截图必须留档，
   由**两个人**分别看一遍。一个人看会把「我知道它是 low 档」读进眼睛里。

两条都满足的最高档如果**不是** `medium`，才动 `ShadowSchema.quality` 的 `.default()`，
并写一条 ADR（下一个可用号是 **0044**——卡面写的 0038 已被
[ADR-0038 编辑器 UI 事件入口的测试范式](adr/0038-编辑器-ui-事件入口的测试范式.md) 占用）。
**不 bump `schemaVersion`**：出厂默认只作用于「文档里压根没写这一项」的情况，它不是字段变更。

### 已经落地的那道护栏（本卡的可交付部分）

改出厂默认**不该动任何一份已经存在的文档**——那份文档里写的 `low` 是作者当年按他那台机器
选的。`packages/schema/test/fixtures/v2/shadow-quality-v2.json` 就是为这件事造的：
**它写的不是默认值**。

⚠ 光有 `fixtures/v2/golden-path-2.json` 守不住：那份里写的就是 `medium`，与默认值一模一样，
所以「保留了原值」和「被默认值覆盖了」在它身上完全同形。两条断言实测过：

| 变异 | 期望 | 实测 |
|---|---|---|
| 出厂默认 `medium` → `low` | 老 fixture **仍然绿**（改默认不影响历史文档） | 绿 · 502 条全过 |
| 迁移里强制把 `quality` 写成 `medium` | fixture 断言**必须红** | 红 · 「每一份 fixture 里写明的 quality，迁移之后逐个不变」 |
