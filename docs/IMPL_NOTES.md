...
| CI 工作流在 GitHub 上真跑一次 | ✅ **已真实执行** | 2026-07-31 推 main 首跑。**第一次是红的**，见下方「T-117 首跑抓到的一条」 | — |
...
| 严重度 | 发现 | 位置 | 处置 |
|---|---|---|---|
| major | **全新 clone 上 `pnpm -r typecheck` 直接失败**：`Cannot find module '@w3/schema'`。每个包都经 `types: ./dist/index.d.ts` 认识兄弟包（ADR-0003：声明由 tsc 出），全新机器上 `dist/` 不存在，所以类型检查一行都没查到就挂了。本机一直是绿的，因为 `dist/` 是上一次构建留下的 | `.github/workflows/ci.yml` · 各包 `package.json` 的 `types` | 已修：CI 里 `build` 提到最前。**这条只有真跑 CI 才会暴露**——它正是「本机全绿」与「干净环境全绿」之间的差 |

> 记这一条的意义不在于它多严重，而在于它证明了这张卡的价值：工作流文件写完、
> YAML 校验过、每条命令本机跑通，仍然是红的。**没跑过的 CI 配置与没有 CI 配置，
> 可靠性是同一档。**
