...
### E18 清偿期间新发现（T-185 / T-186 审查所得，2026-08-02）

清债把 T-176 的登记项修完之余，测试扩展与三视角复查又照出四条。两条当场修掉，
两条如实登记——与 T-176 同一条纪律：**不留没人负责的已知项**。

| 级别 | 处 | 发现 | 处置 |
|---|---|---|---|
| major | 文档 vs 渲染器 | **换承载体类型会丢表面状态**。`/nodes/i/primitive` 的 kind 变更走 `replaceObject`，fresh mesh 拿的是 three 默认值：`castShadow` false、工厂中性灰——盒子换成球，影子没了、材质覆盖掉了，面板上全是对的，到下次全量 load 才恢复。已实测复现（T-185 的快照扩展研究预警，本轮验证） | ✅ **已修**：`apply-patch` 的 primitive 路由在 `setPrimitive` 后重放 `applyNodeShadow` 与材质覆盖；灯不需要（工厂从定义整体重建）。端到端回归测试在 `scene-runtime.test.ts` |
| major | 正向路径 | **材质定义换人时继承参数漂移**（详见 T-185 台账 ②）：`applyParams` 把「缺省=继承源材质」实现成「缺省=不写」，复用的 clone 粘着上一个定义写过的值 | ✅ **已修**（T-185 提交内）：def 换人先从源材质重置可继承参数 |
| minor | 边界 | **整条材质记录的替换绕过转发器的 maps 判断**：`['materials', i]` 级别的补丁（如撤销「删除一条带贴图的材质」）路径里没有 `'maps'`，不触发 `ensureAssets`——贴图能否画出来取决于缓存里碰巧有没有 | 登记。惟一可达路径是撤销材质删除，而 v0.5 的 UI 尚无删除材质入口（与 T-144 登记的可达性一致）。UI 长出删除入口时这条必须一起清 |
| minor | 语义 | **`stopMedia` 停不掉正在 await 的等待**：停播不触发 `ended`，`waitForEnd` 的悬挂等待者也不被 settle，规则链仍要等完剩余 `durationS`（错误事件同理，`MediaElementLike` 未监听 `error`） | 登记。「停播算不算播完」是产品语义决定，不该由一张清债卡拍板；决定后修在 `MediaBus.stop` settle 等待者即可 |
| minor | C3 边缘 | **`resetScene` 对相机的处理两侧不一致**：headless 会把视点复位，`SceneRuntime` 不会。契约测试没盖到这一项，所以从未红过 | 登记。修哪一侧取决于「重置场景该不该动相机」——同上，先裁决再动手 |
...
| minor | 内存 | **清空贴图槽位不会释放显存**。`TextureCache.ensure`（顺带做回收）只在"资产到达"或"材质开始引用贴图"时跑，而**取消**引用两者都不是；字节要等到下一次导入或下一次 `load` 才还回去 | ~~登记不修~~ **T-186 复查：登记时的前提已不成立**——上面那条 blocker 的修复（`/materials/**/maps/**` 全部走慢路）把「清槽位」也送进了 `ensureAssets`，基础纹理自那时起就会被 `retainOnly` 回收，这条登记从未被回头更正。真正残留的是**变体泄漏**：资产还被别的材质引用时，被清槽位的 (资产,采样态) 克隆永远无人清扫（colorSpace/wrap 不同时是真的一张额外 GL 纹理）。T-186 已修：`retainOnly` 按「(资产,采样态) 是否仍被想要」清扫变体，附回归测试与变异检验 |
