# CLAUDE.md · 工程铁律（**历史副本，已停止维护**）

> ⚠ **唯一权威是仓库根目录的 [`CLAUDE.md`](../CLAUDE.md)。这份是它被复制到根目录之前的规划期原件。**
>
> **2026-08-03 发现两份已经分叉**：v1 落地时只更新了根目录那份（当前活动版本 v1.0、必读文档表、
> 停下来问人第 8 条），这份仍停在 v0.5 的口径。分叉不是谁疏忽——**同一份内容存在两处，漂移是时间问题**，
> 这正是 v1 规划 §1.0 反复点名的那个失效形状在文档层的一次实例。
>
> 保留本文件只为留痕（它是 v0 起草时的原件）。**任何时候以根目录版本为准；不要再修改本文件，
> 也不要把它当成规范引用。** 下面的内容按 v0.5 时的状态原样冻结。
