...
| **B15** | `setVar` 写一个未声明的变量 | **恰好一条 `error`，措辞逐字 `写入了未声明的变量「<id>」，忽略`，两个运行时逐字相同，写入不生效**。不静默创建：那会让 `checkIntegrity` 的引用检查失去意义（一条指向不存在变量的规则会自己把它变出来），也违反 C1「状态只进文档」。不降成 warn：一条永远不生效的写入是配置错误，不是可以忽略的提示 |
| **B16** | 编辑器新建 / 删除一个变量 | 预览中的运行时**立即**跟上：新变量取 `default`，**已存在变量的当前值不动**，被删的变量清除。⚠ 这条失效时 `fullRebuildCount` **全程是 0**——路径一直被认领着，所以没有回落、没有告警，功能就是不工作（T-231） |
