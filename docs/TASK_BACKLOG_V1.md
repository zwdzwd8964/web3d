...
### [x] T-213 · 遗留决议清零（ADR-0014 / ADR-0013）
- **依赖** 无 · **预估** 0.2d · **实际** 0.9h
...
- **人工拍板已就位（2026-08-04 · zwdzwd8964）**：ADR-0014 确认销案并署名；ADR-0013 确认
  「全部部署环境支持 WebGL 2」。两条都写进各自 ADR 与 `IMPL_NOTES` 的对应段。
  **规划 §7.2 的 P-1 ~ P-20 二十项里没有任何一项覆盖这两件事**——它们从 v0 挂到今天，
  靠的是没有任何机器在看。
- **交付偏差**（五处，均因验收标准超出独占清单）：
  ① **ADR-0011**——`grep -rn "需人工确认" docs/adr` 实测**命中 2 处**不是 1 处，第二处在
  ADR-0011:48 的「代价」栏，而它自己的状态栏已经是 `Accepted`（自相矛盾）。全 199 张卡
  无人认领它，不改则自测与 H4 永远红；
  ② **ADR-0018 / ADR-0019**——按验收字面口径「非 Accepted 列表为空」，实测**四份**不合格
  （0013「已接受」· 0014「已接受（但需人工确认）」· 0018「已采纳」· 0019「已接受」），
  卡面只处置了两份；
  ③ **`scripts/check-docs.mjs`（新增规则 8）与 `scripts/lib/adr-status.mjs`（新建）**——
  见下条；
  ④ **`docs/TASK_BACKLOG_V1.md` 的 T-458 卡面**——就地更正它的变异 ① 方向，见下条。
- **「替代验收」在本卡之前无处可落。** 卡面写「把 ADR-0014 的状态改回『需人工确认』→
  `check-docs.mjs` 必须报一次」，而七条规则里唯一碰 `docs/adr` 的是规则 3，它**数文件个数、
  不读内容**。决定性证据是现状本身：开工那一刻 ADR-0014:3 正写着「需人工确认」，
  而 `pnpm check:docs` 输出 PASS、exit 0。因此本卡建了规则 8。
- **状态判定是逐字相等，没有同义词表**，这一条是被变异证伪出来的：改成
  「以 `已接受` / `已采纳` 开头」之后，**坏串「已接受（但需人工确认）」当场被判成 Accepted**——
  它就是以「已接受」开头的。四种写法在本卡人工归一了一次，从此解析器故意不宽容。
- **T-458 的变异 ① 方向反了，已就地更正。** 原文「把某条 ADR 的状态改成 Accepted →
  输出必须变化」，而本卡把 31 份全部归一到 `Accepted` 之后，那是恒等操作——照原文跑会得到
  一条**恒为绿**的变异并被记进登记表。
- **验收第二条今天无法执行也无法证伪**：它引用 `scripts/milestone-close.mjs` 的第 7 步，
  而该脚本由 **T-458（W34）** 交付，今天文件不存在。`adr-status.mjs` 抽成共享模块正是为它——
  两份解析器会在有人第五次换写法的那天分叉。
- **自测命令的退出码是反的**：`grep` 有命中（失败态）exit 0、无命中（通过态）exit 1。
  已改写成 `! grep -rn "需人工确认" docs/adr`。
- **自测** `! grep -rn "需人工确认" docs/adr && pnpm check:docs`（**注意取反**，见上）
...
### [x] T-297 · 变异检验登记表与它的机械锁
- **依赖** T-208（`package.json` 同文件）· **预估** 0.6d · **实际** 1.3h
- **③ 的等式改成了包含关系**（`需登记 ⊆ 已覆盖`），不是相等。实测两侧当天就是 **17 ≠ 16**：
  多出来的是 **T-212**——它的「变异检验」栏写着「不适用（合同 / 文档卡）」，却仍然跑了一条
  替代验收并如实登记。按相等实现会 exit 1，**而唯一的修复动作是删掉那行有价值的记录**。
  多登记的打 note 不 fail，缺登记的点名卡号，这才是这把锁该有的激励方向。
- **「实际」列立了四个封闭 token**（`红` / `绿` / `绿→红` / `非变异`）。立之前同一张表按
  「含『绿』字」数得 19、按「含绿且不含红」数得 14——**而 T-458 要按这个数报「本里程碑未转红
  条数」**。`绿→红` 单列一档：并进 `红` 等于把「我们差点漏掉这个缺陷」洗成「一切正常」，
  而这一档在本仓信息量最大（19 条未转红里 8 条属于它）。`非变异` 是补的第四档，七列模板
  原本没给「为留证而记的观察」留位置。**只改了 15 行的首 token，正文一字未动。**
- **自写了转义感知的行切分。** 照抄 `check-docs.mjs:366` 的 naive `.split` 会把四行判成格式
  错误，而那四行正是正文里引用了正则的行（信息量最大的四行）；「修正方向」若搞反就会毁掉证据。
- **卡面变异 ⑤ 只给了登记表一侧的探针**，补了台账那一侧的 ⑤′ 与 ⑤″：需登记集合是从台账算
  出来的，台账读坏时 R1 会安安静静全过。⑤″ 用「文件真实存在但卡头匹配零处」——它绕过
  `existsSync`，是正则写错时的真实形状。
- **⚠ 规划 §7.1 的 G1.0-22 判据指向一个不存在的栏。** 它写「每张标 `[x]` 且『新增测试』非空
  的卡」，而台账里从来没有「新增测试」这一栏（全仓零命中，卡面字段清单见 §0）。按字面实现
  这把锁从第一天起就是装饰品。**按卡面 R1 的判据实现**（「变异检验」栏不含「不适用」），
  并就地更正规划 §7.1 那一格。
...
### [x] T-298 · `CONSTITUTION-EXCEPTION` 到期守卫
- **依赖** T-205（复用 `scripts/lib/exemptions.mjs` 的版本比较）· T-223（`check-constitution.mjs` 同文件）·
  **预估** 0.5d · **实际** 1.1h
- **卡面「今天零命中」是错的**：实测 1 条（`scripts/lib/exemptions.mjs:8` 的 JSDoc）。
  三条 ADR 写的「`grep -rl … scripts/` 无输出」是 **T-205 交付之前**的事实。锚点因此收成带冒号的
  `CONSTITUTION-EXCEPTION:`，散文说明登记成一张**集合相等**的表（今天 3 条）——
  **不许靠正则碰巧躲开**，那样下一处散文会被随机审判。
- **变异 ③ 逻辑上不可能成立，已替换并写明原委。** 实测 `VERSION_LADDER` 七级恰好字典序有序，
  字符串比较与阶梯比较**对 49 组合法级别对给出完全相同的结论**；`v1.10` 根本不在阶梯上。
  替身：新增第五份夹具 `off-ladder.ts.fixture`，变异改成「删掉阶梯成员校验」。
- **卡面 ② 按字面执行会造出一个永远不红的守卫**：`package.json` 是 `"version": "0.0.0"`、
  无 `w3Version`，`isExpired('v1.0','0.0.0')` 恒 `false`。改为 `resolveCurrentVersion()`，
  且**解析结果不在阶梯上就直接红**。顺带登记：仓库已有**三套**「当前版本」算法。
- **⚠ 栏「确认三条到期承诺都能被脚本解析出来」不可直接满足**：三条一条都不在代码里
  （`captureImage()` 还是 T-266 的桩 · benchmark 脚本未创建 · `main.ts:80` 上方无注释）。
  降级为「**五份夹具逐字复刻三条原文**，`--self-test` 证明解析器认得它们」。扫描面显式不含
  `docs/**`——纳入的话 ADR-0022 的「到期 v1.0」在 current=v1.0 时立刻红，与验收 exit 0 冲突。
- **我在正则上方写了那个陷阱，然后照样掉进去了。** ADR-0024 的实例是
  `到期 v1.5（改走 HttpApiProvider）`，贪婪的 `(\S+)` 捕到 `v1.5（改走`——**一条真会到期的
  例外，被专为抓它而写的守卫放行**。`not-due` 夹具逐字复刻那个全角括号补语，正是它抓到的。
- **`<条款>` 段是任意非空文本，不是 `/^C[1-9]$/`。** 四种真实形态：`C5` · `C7` · `§3` · `渲染出口`。
  收成 C 编号会拒掉 ADR-0022 与 ADR-0025 两条真承诺，而「解析不出来 = 红」。
- **夹具用 `.ts.fixture` 后缀 + `SKIP_DIRS` 显式跳过，两层都要**：只靠后缀的话，有人把夹具改名成
  `.ts` 的那天守卫会红在自己的测试数据上，而原因不会显而易见。
- **交付偏差**（两处）：`docs/MUTATIONS.md` · `docs/TASK_BACKLOG_V1.md`（DoD 要求，卡面独占未列）。
- **GUARDS 插在末尾**，T-223 已先落地（同文件），本卡追加不改它那一行。
...
### [x] T-242 · 背景色与曝光控件（未认领缺口补齐）
- **依赖** T-239（同文件 → 排其后）· **预估** 0.5d · **实际** 0.9d
...
### [x] T-249 · 编辑器：单零件 `explodeOffset` 的记录与清除
- ✅ **拍板（2026-08-05，产品负责人）：I22 的现有措辞不动，订正本卡卡面。** 卡面写的
  「逐字引用了记录当前偏移」「唯一一条兜底出路」两处都与实现不符：实测措辞是「建议改用
  轴向模式或为每个零件设置爆炸偏移」，给的是**两条**出路，**而那与 D28 一致**——写错的是
  卡面，不是实现。它是已交付 T-226 的验收面（面向用户的字符串），不因一句卡面描述而改动。
- ✅ **与 T-248 的冲突已裁决**：T-248 让步，工具态下可拖动零件。见 T-248 的留痕。
- **依赖** T-248（`explode-edit.ts` 同文件 → 排其后）· **预估** 1.0d · **实际** 1.0d
...
### [x] T-250 · Picker 的剖切感知
- **依赖** T-243 · **预估** 0.5d · **实际** 0.6d
...
### [x] T-252 · 剖切 × 描边的交互实测（观测卡）
- ✅ **拍板（2026-08-05，产品负责人）：批准新增 I70**，登记为规划 §4.2「编号在这里一次
  分配完」的**一次显式破例**。理由：本卡要观测的「描边不吃裁剪」大概率是 v1.0 的永久已知
  限制（three 的 `OutlinePass` 用 ShaderMaterial 且没设 `clipping:true`），没有机械提示
  等于只在文档里留一句话。**破例仅此一条**：I71 起仍按 [ADR-0020](adr/0020-v1-拆成三级台阶.md)
  登记 v2、不追加。到期版本号 `v1.0`（v1.0 收口时复核这条破例是否仍然必要）。
- **依赖** T-243 · T-240 · **预估** 0.5d · **实际** 1.2d
...
### [x] T-253 · `restart` 参数的语义收口
- **依赖** T-216 · **预估** 0.3d · **实际** 0.4d
...
### [x] T-277 · 对外 API 文档 + 样板宿主页
- ⚠ **返修**（ADR-0042 决策 5）：样板页对同一个容器 mount 了两次，起了两套完整运行时，按钮绑到了溢出到框外的那一个。本地试跑查出，`docs-sync` 只比文本切片挡不住。补 `e2e` 用例 5 + 三条文本断言。
- **依赖** T-274 · **预估** 1.0d · **实际** 0.8d
...
### [x] T-278 · bench 页浏览器级 E2E
- ⚠ **返修**（ADR-0042 决策 1）：`?fast=1` 把一处 v0 就在的潜伏缺陷（`measure()` 墙钟/帧数两把尺子）变成随机必现；而本卡写的 e2e 只查行数/判定合法/值非空，在「平均帧率 = 0.0 fps」的报告上**绿了六次**。已补三条内容断言。
- **依赖** T-208 · **预估** 0.6d · **实际** 0.6d
- **交付偏差** 一处：新建 `e2e/fixtures/scene-package.ts`（零资产 `.w3p` 生成器），并把 T-276 刚写的那份内联生成器换成它。两份 spec 都只需要「一个能打开的包」而不需要走发布流程，各维护一份等于让同一段文档形状在 e2e 里存在两处。
...
- ⚠ **返修**（ADR-0042 决策 1 / 2）：本卡把假 0 的污染面从 2 行扩到 6 行；另有一处与采样无关的独立缺陷——「阴影贴图显存」按灯数而非按显存取最大，永远报最便宜的 low。
...
- ⚠ **返修**（ADR-0042 决策 3 / 4）：判据问的是「文档里有没有刀」而不是「装上平面没有」，于是一把关着的刀产出三行绿色的 `0 ms`；且这一档排在爬坡之后，program 缓存已热，「重编译」那一行结构上恒为 0。
...
### [x] T-284 · 泵组样板里的爆炸 + 剖切编排
- **依赖** T-283 · T-246 · T-251 · **预估** 1.0d · **实际** 1.0d
- 六条规则齐全（开场飞位 / 三级拆开 / 复原 / 剖开 / 合上 / 剖面扫掠），9 种动作、2 个变量、
  2 条动画。`checkIntegrity` 零 error（唯一 warn 是 I70，剖切 + 描边并存的如实告知）。
- **⚠ 未交付一条验收**：「径向爆炸在真实泵组模型上确实有可见效果（E2E 截图 `colourBuckets`
  前后不同）」**没做**——它要浏览器。因此变异 ① 如实登记为**绿**（a 类），与 T-285 一起补。
  「剖开 → 合上两次往返后 `clippingPlanes.length` 回到 0」改在 core 的 `section-layer.test.ts`
  里跑：不需要 GL（C8），而且**读的是渲染器**而不是 `layer.livePlanes`。
- **查出 T-283 的一处真缺陷**：样板规则用了 `setViewpoint`，而**那个动作根本不存在**
  （注册表里是 `moveCamera`）。`checkIntegrity(doc, {actionRefs})` 对它零 error ——
  换成随手编的动作名也照样零 error。**完整性检查不查动作类型存不存在**，这是 T-283
  那条验收的一个洞，反向覆盖交给 T-285。
- **交付偏差** 两处：① 剖切面默认可见性与 T-283 冲突，依本卡验收改回 `false`；
  ② 测试写在 `packages/core/test/runtime/section-layer.test.ts`（既有文件）而不是新建，
  因为渲染器替身与 `setup()` 都在那里。
- ⚠ **合同措辞待确认（本卡的 ⚠ 未闭合）**：由于剖切零新增动作，自动生成的验收用例措辞
  会是「显示对象「水平剖切面」」而不是「打开剖切」。T-212 定的是剖切**能做什么/不做什么**，
  没有涉及这句措辞。生成器本身还不存在（v1.5 的验收材料线），所以不阻塞——但要不要接受
  这句话，是产品负责人的决定：不接受则剖切要多花一张卡（`setSection` 动作 + RefKind 扩容
  + 规则编辑器改动）。
...
### [x] T-289 · 崩溃恢复 E2E 三条
- **依赖** T-288 · **预估** 0.6d · **实际** 0.8d
- 三条全绿，各自 **6.2s / 3.6s / 4.7s**（门槛 30s）。第 1 条的数字是**从横幅上读回来的**，
  正则要求它是一个正整数——文案里写死一个 N 的实现会红。
- **第 3 条故意不用 `?w3LeaseStaleMs=1000`**：那个开关只调过期阈值、不调心跳间隔（仍是 5 秒），
  于是 A 自己的租约会在两次心跳之间就过期、被 B 接管，黄横幅永远不出现。第一次跑就是这么红的。
  **不是产品缺陷，是这个测试开关的作用域**——它为「让崩溃判定快点发生」而造，而第 3 条要的相反。
- **卡面变异 ③ 的预期不成立，如实记**：删掉 `pagehide` 里那次 `releaseLease`，三条全过。
  干净退出时东西都存完了、库里没有草稿，而 `bannerFor` 要求草稿存在。补了一条替补变异 ③′
  （`bannerFor` 不再要求草稿存在 → 第 2 条红），那才是第 2 条真正守着的东西。
- **交付偏差** 一处：`__w3SimulateCrash` 从「停心跳 + 不释放租约」扩成**再加一条「正式保存
  永不返回」**（`crashSimulated` 挂在 project-lifecycle，`useAutoSave` 读它）。不这么做的话，
  伪造的崩溃仍然会把文档存完、把草稿清掉，E2E 第 1 条要跟防抖赛跑。永不 resolve 而不是抛异常：
...
### [x] T-291 · 目标机器 benchmark 实测手册与三机采集（人工供给项）
- **依赖** T-279 · T-280 · **预估** 0.5d 代码侧 + 人工实测另计 · **实际** 0.6d（代码侧）
- **代码侧交付完**：`BENCHMARK.md` 新增「拿到机器之后的 30 分钟」八节（0 准备 / 1 硬件加速前置
  检查 / 2 三个档位 / 3 两个场景 / 4 命名 / 5 交回 / 6 什么算通过 / 7 常见问题）+
  `docs/bench-reports/README.md`。**手册是写给没参与过项目的人的**，全程不需要 Node、
  不需要命令行、不需要联网。
- ⏳ **人工实测部分仍未闭合**（三台机器 × 两个场景 = 六份 JSON）。按 [ADR-0022](adr/0022-G0.5-8-目标机器-benchmark-的挂载方式.md)
  它是 **v1.0 的出口人工验收项 H1**，不阻塞任何编码卡开工。回填位在规划附录 D 的 H1 行。
- **偏离卡面一处，且有实测依据**：卡面写的命名是 `<代号>-<日期>.json`，**已经落地的**
  `apply-bench-report.mjs` 按行首档位正则解析，那个名字它认不出来（实测 exit 1 并点名）。
  手册改用 `<档位>-<机器名>-<场景>-<日期>.json`，六个例子实测全部被接受。
- **替代验收做了机器能做的那一半**：造六份假报告放进临时目录逐个喂给脚本（**一份都不进仓库**），
  验完确认附件A 工作区干净。四条结果记在 MUTATIONS.md。
- **新发现并写进手册**：M3（关硬件加速那一档）按定义就是软件渲染，因此**它不进附件A §7**
  ——回填脚本会拒绝它。它的结论写进 IMPL_NOTES。卡面没写这一条，而不写清楚的话，
  现场跑完三台会得到一份「第三台怎么填不进去」的困惑。
- ⚠ **本卡自己制造并修掉了一次文档损坏**，值得记：回填脚本的替换串里写了一段带正则行尾锚点的
  文字，而 `$` 紧挨着一个反引号——`String.replace` 把它当成替换模式「匹配位置之前的全部内容」，
  于是台账与 BENCHMARK 各被塞进了自己的前半段（台账卡头从 199 变成 293）。**是 `git show --stat`
  的 +2888 行数字露的馅**，不是任何一个守卫。已改用 indexOf + slice 重做，切片没有 `$` 语义。
...
### [x] T-292 · 遗留决议 S3 · 阴影三档实测与出厂默认裁决
- **依赖** T-291 · **预估** 0.4d 代码侧 + 人工实测另计 · **实际** 0.5d（代码侧）
- **结论：维持 `medium`，并把「它是拍的不是测的」这件事明确写下来。** 软渲环境下三档代价与
  真实 GPU 无可比性，所以本卡交付的是**实验设计 + 判据 + 那道护栏**，不是数字。
  判据先写死（平均帧率 ≥ 45 fps 且阴影边缘两人各看一遍），免得看到数字之后再挑标准。
- **护栏是本卡真正的可交付部分**：新建 `fixtures/v2/shadow-quality-v2.json`，**它写的不是默认值**。
  卡面要求的两条变异实测：改出厂默认 → 老 fixture 仍然绿（502 条）；迁移强制覆盖 quality → 断言红。
- **先撞上的一堵墙，值得记**：既有的 `golden-path-2.json` 守不住这件事——它写的就是 `medium`，
  与默认值一模一样，「保留了原值」与「被默认值覆盖了」在它身上完全同形。另加了一条元断言
  「至少有一份 fixture 写的不是出厂默认」，否则主断言会在「所有样本都写 medium」的世界里继续绿。
- **ADR 号更正**：卡面写的 0038 已被 [ADR-0038 编辑器 UI 事件入口的测试范式](adr/0038-编辑器-ui-事件入口的测试范式.md) 占用。
  下一个可用号是 **0044**，且**只有推荐档 ≠ medium 时才需要写**——今天没有数据，所以没写。
- **顺手更新了一条陈旧登记**：IMPL_NOTES 的 U-02「跨标签页锁仍未验证」在 T-287/T-289 已经闭合
  （契约套件两侧四条判定 + E2E 开两个真标签页），改成 ✅ 并写明证据。
- ⏳ **人工实测部分未闭合**，登记为 U-24，与 T-291 同属 v1.0 出口人工验收项 H1。
- **交付偏差** 一处：`packages/schema/src/light.ts` **没动**（卡面写的是「仅当推荐档 ≠ medium」，
  而今天没有推荐档）。
...
- **变异检验** ① **（方向已由 T-213 就地更正）** 把某条 ADR 的状态从 `Accepted` 改成别的 →
  第 7 步的输出**必须变化**。**原文写的是反方向**（「改成 Accepted → 输出必须变化」），
  而 T-213 已把全仓 31 份 ADR 归一到 `Accepted`，「改成 Accepted」自那天起是恒等操作，
  输出不可能变化——照原文跑会得到一条**恒为绿**的变异并被记进 `MUTATIONS.md`
...
|---|---|---|---|---|---|---|
| **M14** ✅ | v1.0 | T-200 ~ T-224 · T-297 · T-298 | 27 | 23.4 | **实测 15.6h** | ✅ **2026-08-04 收口，三条出口条件逐条兑现**：`pnpm verify` 全绿，宪法守卫 5 → 9 条，另加 `check:docs`（九条规则）与 `check:mutations` 两把新锁；**一份真 Draco 泵组在浏览器里被同源解码并出画**（`decoders.spec.ts`，实测取回 `draco_wasm_wrapper.js` 276,778 字节 + `draco_decoder.wasm`，体检 `tris 24` 与未压缩同源件逐项相等）；**断网 CI job 在 GitHub 上绿过一次**（[run 30944876523](https://github.com/zwdzwd8964/web3d/actions/runs/30944876523)）|
