...
| T-290 | ① | `describeRemoval` 在被引用时不带引用描述 | 文案测试红 | 红 · 2 条 | — | 还原 |
| T-290 | ② | 在 `HierarchyTree` 里再写一份 `describeRemoval` | grep 断言红 | 红 · 1 条 | — | 还原。**「只有一处」这条断言本身写成了测试**——否则重构完两份并存也没人知道，而这正是本卡开工时的实际状态：一份在 `removal.ts`，一份在 `HierarchyTree`，两份的问句还不一样 |
| T-290 | ③ | 把 `save` 从 `SHORTCUTS` 里删掉 | 卡面说 `golden-path.spec.ts:176-192`「Ctrl+S 后状态栏变已保存」是**现成的红灯来源** | **绿** · 那条 E2E 照过 | **b** | **卡面这条判断不成立，如实记。** 那条 E2E 改完名字等 10 秒看状态栏，而自动保存的防抖 1.2 秒就自己写完了——`Ctrl+S` 按不按都会变成「已保存」。⚠ 第一次跑这条变异时它也「绿」，但那次是**变异根本没落地**（锚点串带 `\n`，文件是 CRLF，`String.replace` 静默什么都没做）。加了一条「改完必须与原文不同」的断言之后重跑，才拿到上面这个可信的绿 |
| T-290 | ③′ | （替补）同样删掉 `save`，跑单测 | 「Ctrl+S 在画布上保存」红 | 红 · 3 条（含手册生成块与快捷键表两条） | — | 这才是这条变异真正的红灯来源。顺带说明生成块的价值：删一条快捷键会让**手册当场对不上**，而手册是给用户看的那一份 |
| T-290 | ④ | `allowInTextField` 恒 true | 「输入框里按 Ctrl+S 不保存」红 | 红 · 1 条 | — | 还原。**这条断言描述的正是本卡之前的真实行为**：`Ctrl+S` 住在 `useAutoSave` 的第二个 keydown 监听里，不经过 `isTypingInto` |
| T-290 | ⑤ | 加第 13 条快捷键，`group` 给一个不在 `HELP_GROUP_ORDER` 里的值 | 面板覆盖那条红 | 红 · 4 条 | — | 还原。面板**只渲染闭集里的分组**是故意的：自动把未知分组兜到最后看着更宽容，代价是「每个 id 都出现在面板里」那条断言永远绿 |
| T-290 | ⑥ | `applyBlock` 改成 `return text`（`UPDATE_DOCS=1` 什么都不做） | 需要一条「改坏后能修好」的断言，否则绿 | 红 · 1 条 | — | 还原。**「与 SHORTCUTS 一致」那条对它无感**——文档本来就是一致的，什么都不做也一致 |
| T-290 | ⑦ | （非变异，是一次测试竞态的暴露）golden-path ⑧ 的两处采样 | 本卡不该动它 | 非变异 · 加了两个空渲染的组件之后它开始红；**同一份代码连跑三遍，期望值一次 17、两次 12**，两边都出现过 | — | 采样从「固定等一会儿数一次」改成「连续两次相同」+ 终值用 `expect.poll` 重试。**中间那一版只改了前一半，反而更红**：重建过程中的某两帧完全可能相同，而那两帧上模型还没回来——「稳定」不等于「稳定在终值上」 |
| T-299 | ① | `NullAiProvider.enabled` 改成 `true` | 「默认关闭」红 | 红 · 1 条 | — | 还原 |
| T-299 | ② | `suggest()` 从 `throw` 改成 `return []` | **同一条测试的第二个断言**必须红 | 红 · 1 条（与 ① 是同一条用例） | — | 还原。**卡面点名这是本卡唯一会假绿的地方**：两个断言分成两条测试的话，这次变异是绿的——而「插座默认关闭」就退化成一个布尔字段的自证。布尔字段自证不了任何行为，能自证的只有那次调用 |
| T-299 | ③ | 删掉冻结接口表里 `resolveAiProvider` 那一行 | `check-dead-exports` 红且点名它 | 红 · 1 条，逐字点名 `core:resolveAiProvider` | — | 还原。**这一行是本卡与「又一条预留了但没人接」之间的全部差别** |
| T-299 | ④ | 往 `suggest()` 里塞一个 `fetch("/ai")` | 「本目录零网络原语」红 | 红 · 2 条 | — | 还原。插座本身不许有网络能力；接不接得上是 v2 的事 |
| T-299 | ⑤ | `resolveAiProvider` 里加一条 `process.env` 分支 | 「不读环境变量」红 | 红 · 1 条 | — | 还原。有一条探测，「默认关闭」就变成「默认取决于部署环境」，而内网那台机器上没人会去验证它 |
| T-299 | ⑥ | （非变异，是第三次撞同一堵墙）5 个零调用者导出 | 卡面要求往四列豁免表加**一行** | 非变异 · 两处与卡面不符：① 守卫点名的是 **5 个**符号（成员 + `resolveAiProvider`），卡面写的 `AiProvider` 本身**不是**孤儿（它在本文件内被 `implements` 与形参用到，符号级扫描认在文件内的使用）；② `MAX_EXEMPTIONS` 仍是 19/19，只能降不能升 | — | 走 §4.8 + 冻结接口表，与 T-286（ADR-0043 裁决甲）、T-287 同一条路。**两把棘轮一格都没动** |
| T-299 | ⑦ | （非变异，是守卫先于我发现）`new NullAiProvider()` | 应当被 C7 的 R1 拦下 | 非变异 · 红 1 条：「未申报的 provider 构造点：NullAiProvider」 | — | 申报进 `PROVIDER_SITES`。**这正是 R1 用集合相等而不是计数的价值**——一个新构造点凭空出现，计数式的规则只会说「至多一个」仍然成立 |
| T-291 | — | （文档卡，卡面写明变异检验不适用） | 替代验收是「找一个没参与的人照做一遍」 | 非变异 · 人找不到，**但那句话里机器能验的那一半验了**：手册第 4 节逐字给出的六个文件名，脚本是不是真的都认得 | — | 造六份假报告放进临时目录（**一份都不进仓库**），用 `apply-bench-report.mjs` 逐个喂进去 |
| T-291 | ① | （替代验收 1）六个例子名 | 应当零条「文件名不合规」 | 非变异 · **0 条**，六个全认 | — | — |
| T-291 | ② | （替代验收 2）M3 那份标了 `capability.level: software` | 应当被拒绝 | 非变异 · exit **1**，拒绝理由里逐字写着「软件渲染」 | — | 手册第 6 节据此写明「M3 不进附件A §7」 |
| T-291 | ③ | （替代验收 3）按**卡面字面**的 `<代号>-<日期>.json` 造一个名字 | —— | 非变异 · exit **1**，被点名「文件名要写成 `<档位>-<代号>.json`」 | — | **这条是本卡偏离卡面写法的全部依据**：卡面那个命名，已经落地的脚本认不出来。手册要产出脚本吃得下的东西，否则「合规命名」四个字没有落点 |
| T-291 | ④ | （替代验收 4）全程跑完之后 | 附件A 一个字都不许被写进去 | 非变异 · `--check` PASS，且 `git status` 显示附件A **干净** | — | 假数字没有泄进合同附件 |
| T-292 | ① | 出厂默认 `medium` → `low` | 老 fixture **必须仍然绿**（改出厂默认不该影响历史文档） | **绿** · 502 条全过，如卡面所期望 | **c** | 还原。**这是一条对照变异，期望的就是绿**：出厂默认只作用于「文档里压根没写这一项」的情况。归 c（被测代码本来就是对的）而不是 a，依据是**可证的**——真要是迁移会覆盖 quality，变异 ② 会红，而 ② 确实红了。所以覆盖存在，绿是真的 |
| T-292 | ② | v2→v3 迁移里强制把 `quality` 写成 `medium` | 那条 fixture 断言**必须红** | 红 · 「每一份 fixture 里写明的 quality，迁移之后逐个不变」 | — | 还原 |
| T-292 | ③ | （非变异，是本卡造断言时先撞上的一堵墙）用既有的 `golden-path-2.json` 当回归样本 | 它应当能守住 ② | 非变异 · **守不住**：那份文档里写的就是 `medium`，与默认值一模一样，「保留了原值」和「被默认值覆盖了」在它身上完全同形 | — | 新造 `fixtures/v2/shadow-quality-v2.json`，**它写的不是默认值**（`low`）。另加一条元断言「至少有一份 fixture 写的不是出厂默认」，否则 ② 会在一个「所有样本都写 medium」的世界里继续绿 |
| T-292 | ④ | （非变异）阴影三档在目标机器上的代价 | 卡面要求实测 | 非变异 · **测不了**：本仓库开发与 CI 只有 SwiftShader 软渲，三档代价与真实 GPU 无可比性 | — | 登记为 IMPL_NOTES 的 **U-24**，并把实验设计与判据先写死（免得看到数字之后再挑标准）。与 T-291 同属出口人工验收项 H1 |
| T-293 | ① | 穿越判断退化成 `rawUrl.includes('..')` | `%2e%2e` 那条红 | 红 · **2 条**：`%2e%2e` 那条，**外加「文件名里带 .. 的合法文件不被误伤」那条** | — | 还原。第二条红是意外收获——`includes('..')` 不只是漏掉百分号编码，它还会把 `/assets/index..hash.js` 这种合法产物名一起拒掉。**一个既漏又误伤的判据** |
| T-293 | ② | nginx 模板的 `types` 加一条 `video/mp4 mp4;`，不同步 serve.mjs | MIME 锁红 | 红 · 1 条，并把差集打出来 | — | 还原。断言是**两个方向都查**：只查一边的话，「nginx 加了 serve 没加」与「serve 加了 nginx 没加」里总有一种是隐形的 |
| T-293 | ③ | SPA 回退对 `/player/` 也返回根 index.html | 第 2 条红 | 红 · 3 条（纯函数 1 + 起真服务器 1 + 离线包第 3 步 1） | — | 还原。**夹具里两份 index.html 的内容必须可区分**，否则这条变异是绿的——两个都返回 200、都是 html |
| T-293 | ④ | 往 `editor/dist/index.html` 插一条 Google Fonts 外链 | 离线包第 7 步必须红 | 红 · 第 7 步逐字点名「产物里有外链」 | — | 还原。**这是 C6 在部署产物上的最后一道复检**，它必须真的会响；CI 里也写了同形状的反向证据步骤，并且要求红的必须是第 7 步 |
| T-293 | ⑤ | `SHA256SUMS` 里改一位哈希 | 第 1 步红 | 红 · 逐个文件点名 | — | 还原 |
| T-293 | ⑥ | （非变异，是自检抓到的真缺陷）`serve.mjs` 的入口判定 | 应当能直接 `node deploy/serve.mjs` 跑起来 | 非变异 · 第 3 步红「serve.mjs 8 秒内没起来」 | — | `new URL(import.meta.url).pathname` 返回的是**百分号编码**路径，而这个仓库的目录名里有空格 → 判定恒 false，`node deploy/serve.mjs` **静默什么都不做就退出**。改用 `fileURLToPath`。⚠ **单测抓不到它**：单测 import 这个模块，走的正是判定为 false 的那一支 |
| T-293 | ⑦ | （非变异，自检抓到的第二件事）响应没有 `Content-Length` | 第 3 步应当读得到 body | 非变异 · 第 3 步红，body 读出来是 `3\r\nok\r\n0` —— 分块传输编码 | — | 每条响应都补上 `Content-Length`。这不只是让测试好写：不带的话浏览器下载一个 80 MB 的模型没有进度总量，而一部分内网代理会把分块响应整个缓冲下来再转发 |
| T-293 | ⑧ | （非变异，环境）第 6 步 `tar -tf C:\…\image.tar` | 应当列出包内容 | 非变异 · 红「tar: Cannot connect to C: resolve failed」——GNU tar 在 Windows 上把 `C:\…` 当成 `host:path` | — | 改成在输出目录里用相对文件名 |
| T-296 | D-02① | 修法放宽成无条件 `return true` | 报警器不许被吞掉 | 红 · 「祖先还活着却已经不在图里 → 仍然回落」 | — | 还原。这条盯的是**修得太松**：把「图与文档本来就不同步」也一起放行，等于把 `fullRebuildCount` 这个铁律 11 的报警器拆了 |
| T-296 | D-02② | `removedWithAncestor` 恒 false（等于没修） | 主断言红 | 红 · 「删掉阀盖及其 4 颗螺栓，fullRebuildCount 仍是 0」 | — | 还原。这条盯的是**修得太紧** |
| T-296 | D-02③ | 判据退化成「有父节点就算」 | 报警器红 | 红 · 同 ① | — | 还原。判据必须是「祖先**也从 next 里消失了**」，不是「有祖先」 |
| T-296 | D-02④ | 去掉父链的成环上界 `guard` | 成环文档不许转死 | 绿→红 · 第一版夹具没走进环，换掉之后才红 | **a** | 第一版夹具把环上两个节点**都**从 next 里拿掉了，于是父链第一步就短路返回，**根本没走进环**——一条看起来在测成环、实际一步都没走的用例。改成「环上的祖先全都活着，只删一个挂在环下的节点」之后才红。⚠ 超时设成 2 秒是断言的一部分：少了它这条**不是转红，是挂住** |
| T-294 | ① | 单边 · 只把播放器侧的画布 resize 成 640×360 | 双向比较必须红 | 红 · 「两侧的热点绘制序列不一致」 | — | 还原。这条证明 ops 真的进了比较面，而不是两个空数组在相等 |
| T-294 | ② | 对称 · 两侧同时让 sprite 层不画任何东西 | 双向仍绿，**必须由自检抓到** | 红 · **而且红的是自检**：「热点一个 op 都没画出来 —— 两侧的空数组相等，证明不了任何事」 | — | 还原。**这是 ADR-0019 那条教训的机器证明**：双向比较对这次变异完全无感，抓住它的是那条断绝对量的自检 |
| T-294 | ③ | 对称 · 把 `SAMPLE_CLIP.name` 改成 `NoSuchClip` | 自检应当红 | **绿** | **c** | **这个变异本身是坏的，不是被测代码的问题。** 那个常量同时被 GLB 构造与文档的 `clipName` 引用，改它等于两边一起改——链没断，片段照样找得到。归 c 之前自问过「是不是在给 a 找台阶」：证据是 ③′ 用同一条断言当场红了 |
| T-294 | ③′ | 对称 · 只把文档的 `clipName` 改成不存在的名字（真的断链） | 自检必须红 | 红 · 「playAnimation 必须真的挂起了 ≥ 片段时长」 | — | 还原 |
| T-294 | ④ | （非变异，是自检自己的缺陷）第一版「动画真的播了」的写法 | 应当盯 imported 那条链 | 非变异 · **它稳定地量到了黄金路径那条 1.2 秒的 tween**——`results.find(r => r.steps.some(...))` 找到的是先出现的那条规则 | — | 改成按 `ruleId === 'rl_t294expl'` 定位。**一条看起来在测新路径、实际在测旧路径的断言**，而它在两种情况下都稳定返回 1200ms，是实测把它揪出来的 |
| T-294 | ⑤ | （非变异，脚本时长算错）末步留到 9800 | 两侧轨迹应当一致 | 非变异 · 红：编辑器报 `rl_t294expl aborted`，播放器**一条都不报** | — | 链从 8600 起、0.8+0.5=1.3s，最早 9900 才结束。末两步改到 10200 / 10600。**红得对**：那是两侧对同一次 `stop()` 拆台的不同反应，不是功能分叉 |
| T-294 | ⑥ | （非变异，浮点）两侧 ops 的投影坐标 | 应当逐项相等 | 非变异 · 差在小数点后第 9 位（351.71131510781373 与 351.71131581744766） | — | 按 **0.001 像素**取整。取整粒度是有代价的（它同时让 0.001 像素以内的真实分叉隐形），所以取的是一个远小于「一个像素」的数，不是图省事的整数 |
| T-294 | ⑦ | （第 4 组 · 卡面最看重的一条）把 E2E 里的 `__w3DevPositionOf` 换成读文档 | E2E **必须仍然红** | 红 · 红在「**这一条是整个文件的重点。** 位置必须真的变了」那一句上 | — | 还原。这条证明 E2E 测的是**渲染器**不是文档——爆炸每帧改三对象、从不写文档，读文档的断言在功能整个被删掉时也成立（T-176 抓到的 `setLight` 假绿的同一形状） |
| T-294 | ⑧ | （非变异，我自己写出来的一条空转断言）「文档里的值没动」那一条 | 应当能证明读文档是空转 | 非变异 · **第一版恒真**：把读到的值存进另一个变量，再拿这两个变量相比 | — | 改成爆炸**前后各读一次**再比。**写测试的人也会写出空转断言**，这就是一个 |
| T-294 | ⑨ | （非变异，两处选择器猜错）`.tree-row__vis` 与直接找爆炸滑杆 | E2E 应当能跑起来 | 非变异 · 红：class 实际是 `.tree-row__eye`；滑杆要**先在下拉里选中分组**才渲染 | — | 照实际 DOM 改。**这不是缺陷，是流程**——下拉默认是「爆炸预览：关」 |
| T-294 | ⑩ | （bench 三档的单测）`gradePostFx` 采样不足 / 越界 / 整轮没跑到 | 各出对应的行 | 红→绿 · 五条新单测 | — | 报的是**相对代价**不是绝对帧率（绝对值只在这一台机器上有意义）；采样不足报「未测到」而不是 0%（ADR-0042 的同一条纪律，而 0% 会被读成好消息） |
